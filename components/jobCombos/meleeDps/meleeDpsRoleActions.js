...
const MeleeDpsRoleActions = ({ jobName, defensiveActions, roleActions, jobSpecificActions }) => {
  ...
  const renderJobSpecificDiv = () => {
    if (jobName === 'monk') {
      return (
        <div>
          <span className={styles.info}>Monk has a few job abilities that help them. <b>Riddle of Earth</b> reduces damage taken. Use in raidwide. <b>Riddle of Fire</b> increaes damage dealt by 15% for 20 seconds. Use during burst phase. <b>Riddle of Wind</b> reduces auto attack delay by 50%.</span>
          <span className={styles.info}><b>Brotherhood</b> increases damage for the party, and it gives a buff, Meditative Brotherhood that allows monks to open a chakra when using a skill under the buff. <b>Mantra</b> gives the party increase hp recovery via healing actions. Use during raidwide to help healers bring the party back to full.</span>
        </div>
      )
    } else if (jobName === 'ninja') {
      return (
        <div>
          <span className={styles.info}>Ninja does not necessarily have a raidwide mitigation or party buff, but Ninjas will inflict a vulnerability on the enemy, usually boss to start the burst phase for the party. <b>Suiton</b> - <b>Trick Attack</b> - <b>Mug</b> will increase damage you do by 10% and increase the target&apos;s damage taken by 5%.</span>
        </div>
      )
    }  else if (jobName === 'dragoon') {
      return (
        <div>
          <span className={styles.info}>Dragoon has a few party buffs. <b>Battle Litany</b> increases critical hit rate of party by 10%, use at start of burst. <b>Dragon Sight</b> allows the Dragoon to choose a party member and increase damage dealt for both, 10% for user and 5% for party member targeted. <b>Dragon Sight</b> also makes the dragon not have any positional requirements on attacks for those 20 seconds.</span>
        </div>
      )
    } else if (jobName === 'reaper') {
      return (
        <div>
          <span className={styles.info}>Reaper has <b>Arcane Crest</b>. Grants a barrier to the reaper that when he is used up, it will grant a buff, Crest of Time Returned to the party. This will gradually restore the partys HP. Use during raidwide.</span>
          <span className={styles.info}><b>Arcane Circle</b> is the damage buff for the party, use during burst phase.</span>
        </div>
      )
    }
  }
  ...
  return (
    <div className={classPageStyles.tankDefensiveActions}>
      ...
      <div className={styles.info}>Feint should be used off cooldown, don&apos;t overlap it with our party members! True North should be used when you cannot attack from the correct position, for example when resolving a mechanic.</div>
